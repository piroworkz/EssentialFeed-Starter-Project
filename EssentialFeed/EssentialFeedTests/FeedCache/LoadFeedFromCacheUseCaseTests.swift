...
final class LoadFeedFromCacheUseCaseTests: XCTestCase {
    ...
    func test_load_deliversCachedImagesOnNonExpiredCache() {
        ...
        expect(sut, toCompleteWith: .success(feed.domain)) {
            store.completeRetrievalWithFoundCache(feed.local, timestamp: nonExpiredTimestamp)
        }
    }
    ...
    func test_load_deliversNoImagesOnExpiredCache() {
        ...
        expect(sut, toCompleteWith: .success([])) {
            store.completeRetrievalWithFoundCache(feed.local, timestamp: expiredTimestamp)
        }
    }
    ...
    func test_load_hasNoSideEffectsOnExpiredCache() {
        let feed = uniqueImageFeed()
        let fixedCurrentDate = Date()
        let expiredTimestamp = fixedCurrentDate.adding(days: -7).adding(days: -1)
        let (sut, store) = buildSUT(currentDate: { fixedCurrentDate })
